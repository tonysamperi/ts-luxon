import {Duration, UnparsedDurationObject} from "../../src";

const dur = (vals: UnparsedDurationObject = {
    years: 1,
    months: 2,
    weeks: 1,
    days: 3,
    ...
    Duration.fromObject(vals, {locale: "en-US"});
...
test("Duration#toHuman formats out a list", () => {
    expect(dur().toHuman()).toEqual(
        "1 year, 2 months, 10 days, 4 hours, 5 minutes, 6 seconds, 7 milliseconds"
    );
});
...
test("Duration#toHuman accepts a listStyle", () => {
    expect(dur().toHuman({listStyle: "long"})).toEqual(
        "1 year, 2 months, 10 days, 4 hours, 5 minutes, 6 seconds, and 7 milliseconds"
    );
});

test("Duration#toHuman accepts number format opts", () => {
    expect(dur().toHuman({unitDisplay: "short"})).toEqual(
        "1 yr, 2 mths, 10 days, 4 hr, 5 min, 6 sec, 7 ms"
    );
});
...
test("Duration#toHuman accepts hiding of zero values", () => {
    expect(
        Duration.fromObject({
            ...
            seconds: 6,
            milliseconds: 0
        }).toHuman({showZeroes: false})
    ).toEqual("1 anno, 7 giorni, 4 ore e 6 secondi");
});
